...
bool executable(const std::string& str, const std::string &PATH){
...
void CommandLine::Check(Server *server, const std::string &path){
	for(int i=0;i<(int)cmd.size();i++){
		if(executable(cmd[i][0], path) == false){
    ...
    for(int i=0;i<(int)cmd.size();i++){
        if(cmd[i].global_pipe_stdin != -1 && !server->global_pipe.Find(cmd[i].global_pipe_stdin)){
    ...
    for(int i=0;i<(int)cmd.size();i++){
        if(cmd[i].global_pipe_stdout != -1 && server->global_pipe.Find(cmd[i].global_pipe_stdout)){
...
Pipe& PipeLine::operator [] (int n){
	if(Find(n) == false) p[n] = Pipe(), p[n].Create();
	return p[n];
}
bool PipeLine::Find(int n){
    return p.find(n) != p.end();
}
void PipeLine::Destruct(int n){
    if(Find(n))p[n].Destruct();
    p.erase(n);
}
...
bool PipeSet::Find(int x, int y) {
	...
	if(p[x].Find(y) == false) return false;
	return true;
}
