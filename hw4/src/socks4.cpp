...
#include <unistd.h>
using namespace std;
Socks4Server::Socks4Server(int _port): port(_port){}
...
void Socks4Server::Handler(int clifd){
    char buf[BUF_SIZE]  = {0};
    int n = read(clifd, buf, BUF_SIZE);
    if(n < 8) exit(0);
    Socks4Request req(buf);
    //check firewall
    if(req.cd == 0x01)
        ...
        for(int i=0;i<(int)fds.size();i++){
            if(FD_ISSET(fds[i], &rfds)){
                ...
                if(n <= 0){
                    printf("%d %d close\n", i, fds[i]);
                    FD_CLR(fds[i], &n_rfds);
                    if(fds[i] == dstfd) select_end = true;
                    continue;
                }
...
int main(){
    ...
    Socks4Server socks4(SOCKS4_PORT);
    socks4.Connect();
    socks4.Run();
    return 0;
}
